...
<div class="mail-container">
    ...
    <div class="main-content-wrap">
        @php
            $expire_date = $data->created_at->addMonth(1)->format("Y-m-d h:i:s");
        @endphp
        <p>{{__('Hello')}}</p>

        <p>{{__('Your monthly donation payment to for')}} "{{optional($data->cause)->title}}" {{__('is pending')}} </p>
        <p>{{__('Please donate again clicking bellow payment button')}}</p>

        <div class="price-wrap">{{amount_with_currency_symbol($data->amount)}}</div>
        <br><br>

        <a class="donate_button" href="{{ route('frontend.donation.recurring.separate.page',$data->recuring_token) }}" target="_blank">{{__('Donate Now')}}</a>
