...
<div class="mail-container">
    ...
    <div class="main-content-wrap">
        <p>{{__('Hello')}}</p>
        @if($type == 'customer')
        <p>{{__('A payment from')}} {{$data->name}} {{__('was successful. Donation Log ID')}} #{{$data->id}} ,{{__('Donated cause')}} "{{optional($data->cause)->title}}" {{__('Paid Via')}} {{ucfirst(str_replace('_',' ',$data->payment_gateway))}}</p>
            <br>
            @if(!empty($data->gift))
                <strong>{{__('(You will get donation gift very soon according your provided address)')}}</strong>
            @endif
        @else
            <p>{{__('You get payment from')}} {{$data->name}} {{__('For donation log ID') .'#'}} {{$data->id}}, {{__('donated cause')}} {{optional($data->cause)->title}} {{__('paid via')}} {{ucfirst(str_replace('_',' ',$data->payment_gateway))}}</p>
        @endif
        ...
        <table>
            ...
            @if(!empty($data->gift))
                ...
                <tr>
                    <td>{{__('Gift Delivery Date : ')}}</td>
                    <td>
                        {{optional($data->gift)->delivery_date ?? ''}}
                    </td>
                </tr>
                ...
                <td>
                    ...
                    @if(!empty($all_custom_fields))
                        @foreach($all_custom_fields ?? [] as $key=> $field)
                            <small class="d-block"><strong class="text-dark ">{{ ucfirst($key) . ' : ' }}</strong>{{$field}}</small>
                            <br>
                        @endforeach
                    @endif
                </td>
