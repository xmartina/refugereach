                    ...
                    <tr>
                        <td scope="row">
                            <div class="user-dahsboard-order-info-wrap">
                                <h5 class="title">
                                    @if(!empty($data->event))
                                        <a href="{{route('frontend.events.single',optional($data->event)->slug)}}">{{$data->event_name}}</a>
                                    @else
                                ...
                                @if(optional($data->log)->package_gateway == 'manual_payment')
                                    <small class="d-block">{{__(' Manual Payment Attachment :')}}<a class="btn btn-info btn-sm pull-right" href="{{url('assets/uploads/attachment/'.optional($data->log)->manual_payment_attachment)}}" target="_blank">{{__('View Attachment')}}</a></small>
                                @endif
