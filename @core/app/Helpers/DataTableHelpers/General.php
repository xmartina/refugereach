...
HTML;
    ...
    public static function cloneIcon($action,$id){
        $csrf = csrf_field();
        $clone_text = __('clone this to new draft');
        return <<<HTML
<form action="{$action}" method="post" class="d-inline">
{$csrf}
    <input type="hidden" name="item_id" value="{$id}">
    <button type="submit" title="{$clone_text}" class="btn btn-xs btn-secondary btn-sm mb-3 mr-1"><i class="far fa-copy"></i></button>
</form>
HTML;
...
HTML;
    ...
    public static function viewAttachment($data){
		...
		$attachment_text = __('Bank Transfer <br> Attachment');
        return <<<HTML
